        ...
        return (<>
            <div className="translate__title_section">
                ...
                <div className="tabs">
                    ...
                    {collectionModal ?
                        <CollectionWordModal parts={parts} word={data} hideModal={closeCollectionModal}/> : null}
                    <motion.button
...
const WordTranslateBlock = ({data, closeFavouriteModal}) => {
    ...
    const onLoadTranslateList = (translations) => {
        const newData = translations.map((translation, i) => {
            const synonyms = translation.syn?.map((syn, i) => {
                return <div key={i} onClick={() => onSubmitWord(syn.text)}
                            className="translate-variant">{syn.text}{syn.gen ? (<span> {syn.gen}</span>) : null}</div>
            })
            ...
            return (
                <li key={i}>
                    <div onClick={() => onSubmitWord(translation.text)}
                         className="translate-variant">{translation.text}{translation.gen ? (
                        <span> {translation.gen}</span>) : null}</div>
                    {synonyms}
    ...
    return (
        <div className="translate-word__block">
            <div className="text">
                <h3>{data.text}</h3>
                <div className="transcription">{data.ts ? `[${data.ts}], ` : null}{data.pos ? (
                    <span>{data.pos}</span>) : null}</div>
