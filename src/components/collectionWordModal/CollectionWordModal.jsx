...
const CollectionWordModal = ({ hideModal, parts, word }) => {
    ...
    const { data: chosenCollections = [], isSuccess: isSuccessChoice, isError: isErrorChoice, isLoading: isLoadingChoice, isFetching: isFetchingChoice } = useGetCollectionWordQuery(query);
    ...
    const toggleChosenCollection = (current, data, collectionId) => {
        ...
        if (cols_id.includes(collectionId)) {
            deleteWord({ path: chosenCollection.col_word_path, wordId: chosenCollection.id });
            current.classList.add("fa-regular");
            current.classList.remove("fa-solid");
        } else {
            let word = {
                part: data.pos,
                translation: data.tr[0].text,
                word: data.text
            }
            if (!isEnglish(word.part[0])) {
                word = {
                    part: formatPartOfSpeech(data.pos),
    ...
    const onLoadCollections = () => {
        return (collection === "" ? collections : collections.filter(({ title }) => title.toLowerCase().includes(collection))).map(({ id, title }) => {
            return (
                <li className='collection' key={id} onClick={(e) => toggleChosenCollection(e.currentTarget.querySelector('i'), chosenWordPart, id)}>
    ...
    return (
        <>
            {choiceWordCollectionModal ? createPortal(<WordPartsModal partsWord={formatPartsWord} hideModal={closeChoiceWordCollectionModal}><h5>Выберите, слово какой части речи вы хотите добавить в коллекцию:</h5></WordPartsModal>, document.querySelector(".translate__title_section .tabs")) : null}
            ...
            <div className="collection-word__modal" style={choiceWordCollectionModal || addingCollectionModal ? { display: "none" } : { display: "block" }} onClick={(e) => e.stopPropagation()}>
